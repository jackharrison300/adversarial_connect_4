...
###############################################################################
    def value(player, board, depth_limit, alpha=-math.inf, beta=math.inf):
        ...
        next = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        if (next == max_player):
            return max_value(next, board, depth_limit, alpha, beta)
        else:
            return min_value(next, board, depth_limit, alpha, beta)
...
###############################################################################
    ...
    def exp_value(player, board, depth_limit):
        ...
        for child in children:
            v += (1.0 / len(children)) * value(player, child[1], depth_limit - 1)
        return v
